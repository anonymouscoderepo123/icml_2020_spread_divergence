...
def py_x(x, sigma, opt):
    ...
    with tf.variable_scope('py_x', reuse=tf.AUTO_REUSE):
        if opt['dis']=='one_one':
            noisy_data = one_one_conv(x)
            sigma_diag=tf.ones_like(x)*(sigma)
            py_x_dis = tfd.MultivariateNormalDiag(loc=noisy_data, scale_diag=sigma_diag, name='py_x')
        elif opt['dis'] == 'injective':
            noisy_data = injective_conv_spread(x)+x
            sigma_diag=tf.ones_like(x)*(sigma)
            py_x_dis = tfd.MultivariateNormalDiag(loc=noisy_data, scale_diag=sigma_diag, name='py_x')
            
        elif opt['dis'] == 'deep_injective':
            noisy_data = injective_deep_conv_spread(x)+x
            sigma_diag=tf.ones_like(x)*(sigma)
            py_x_dis = tfd.MultivariateNormalDiag(loc=noisy_data, scale_diag=sigma_diag, name='py_x')
            
        elif opt['dis'] == 'injective_seperate':
            noisy_data = injective_conv_spread_sperate(x)+x
            sigma_diag=tf.ones_like(x)*(sigma)
            py_x_dis = tfd.MultivariateNormalDiag(loc=noisy_data, scale_diag=sigma_diag, name='py_x')
            
        elif opt['dis'] == 'learned_cov':
...
def main(opt, dataset):
    ...
    with tf.variable_scope("main", reuse=tf.AUTO_REUSE):
        ...
        if opt['low_variance'] == True:
            kl_logqy_z = kl_qy_z_dis.log_prob(kl_py_x_mean)
        else:
        ...
        if opt['lr_decay']:
            ...
            optimizer_kl = tf.train.AdamOptimizer(learning_rate=learning_rate,beta1=0.5,beta2=0.999)
        else:
    ...
    for t in range(1, opt['training_epoch'] + 1):
        ...
        if opt['dis']!='gauss':
            if t>=opt['noise_start_epoch'] and (t-1) % opt['noise_learning_period'] == 0:
                    noise_kl_list = []
                    for it in range(0, opt['injective_round']):
                        data_ids = np.random.choice(train_size, opt['batch_size'], replace=False)
                        X_mb = dataset.data[data_ids].astype(np.float)
